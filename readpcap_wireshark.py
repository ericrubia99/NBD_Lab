...
'''
file_name = glob.glob("./*.pcap")[0] #here we are selecting the file
print("Working with: ", file_name) 
pcap = pyshark.FileCapture(file_name) #now here there are all the pcap files and then pcap[0] in the prompt
'''
...
def extract_Info_pckt(file_name ): #lista_packet_ICMP
    ...
    #this are all the features we eant to extract, ds_field is all the part in the IP
    #If we want to extract mac addresses we should add them here
    ...
    for packet in pcap:
        ...
        if 'IP' in packet : #look, the attention is on packets in the 3rd layer!
            
            
            #Label
            values.append(packet.ip.dsfield_dscp) #info about priority 
            dscp.append(packet.ip.dsfield_dscp)
            #Features
            
            #Header Length
            values.append(int(packet.ip.hdr_len)) #avaible in the ip layer
            ...
            if "UDP" in packet:
                ...
                values.append(-1)
                values.append(-1)
                #this is for the ARP PROTOCOL which is not working with layer4
...
#dataFrame = extract_Info_pckt(file_name) #calling the function and generate the dataframe
...
#dataFrame.to_pickle("PacketDataframe.pkl") #save the dataframe in a pickle file

#sys.exit("Error message - Data created") #our usually error message
